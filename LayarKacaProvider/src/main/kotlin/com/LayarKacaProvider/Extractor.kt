...
// =========================================================================
// MESIN SERVER PROXY LOKAL (FIX FATAL NETWORK BUG & EOF EXCEPTION)
// =========================================================================
object HydraxProxy {
    var port: Int = 0
    ...
    fun start() {
        ...
        try {
            ...
            println("HydraxProxy: Berhasil menyala di port $port")
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    private fun handleClient(client: Socket) {
        ...
        try {
            ...
            // FIX 1: URL Decode agar simbol '%' kembali normal
            val path = URLDecoder.decode(parts[1], "UTF-8")
            if (!path.contains("?url=")) return
            
            // FIX 2: Parser Parameter Aman (Mencegah padding == terhapus)
            ...
            // Decode Base64 dengan mode NO_PADDING agar bebas dari crash IllegalArgument
            val realUrl = String(Base64.decode(encodedUrl, Base64.URL_SAFE or Base64.NO_WRAP or Base64.NO_PADDING))
            ...
            println("HydraxProxy: Mendapat Request Range = $rangeHeader")
            val reqStart = rangeHeader?.replace("bytes=", "")?.split("-")?.get(0)?.toLongOrNull() ?: 0L

            // FIX 3: Manual Redirect Follower (OkHttp suka membuang Range header jika auto-redirect)
            var currentUrl = realUrl
            var redirects = 0
            val httpClient = app.baseClient.newBuilder()
            ...
            while (redirects < 10) {
                val request = Request.Builder()
                    ...
                    .apply {
                        // Memaksa Range header ikut terus meskipun di-redirect ke node server lain
                        if (rangeHeader != null) header("Range", "bytes=$rangeHeader")
                    }
                    .build()
            ...
            if (response == null || (!response!!.isSuccessful && response!!.code != 206)) {
                println("HydraxProxy Error: CDN menolak dengan HTTP ${response?.code}")
                output.write("HTTP/1.1 500 Internal Server Error\r\nConnection: close\r\n\r\n".toByteArray())
                return
            }

            val code = response!!.code
            println("HydraxProxy: Final Status dari CDN = $code")
            
            var actualOffset = 0L
            if (code == 206) {
                val contentRange = response!!.header("Content-Range")
                actualOffset = contentRange?.substringAfter("bytes ")?.substringBefore("-")?.toLongOrNull() ?: reqStart
            } else if (code == 200) {
                actualOffset = 0L
            } else {
                actualOffset = reqStart
            }
            ...
            if (actualOffset > 0 && actualOffset < 65536) {
                cipher.update(ByteArray(actualOffset.toInt()))
            }
            ...
            println("HydraxProxy Fatal Error: ${e.message}")
            try { output?.write("HTTP/1.1 500 Server Error\r\n\r\n".toByteArray()) } catch(ex: Exception){}
        } finally {
        ...
        try {
            ...
            mp4Sources?.forEach { src ->
                ...
                if (path.isNotEmpty() && baseUrl.isNotEmpty()) {
                    ...
                    // FIX Base64 Encode: Menggunakan mode NO_PADDING mencegah padding "==" yang bikin crash
                    val encodedUrl = Base64.encodeToString(srcUrl.toByteArray(), Base64.URL_SAFE or Base64.NO_WRAP or Base64.NO_PADDING)
