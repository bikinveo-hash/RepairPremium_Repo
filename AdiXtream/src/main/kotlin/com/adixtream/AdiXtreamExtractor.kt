...
object AdiXtreamExtractor : AdiXtream() {
    ...
    suspend fun invokeKisskh(
        ...
        val apiUrl = "https://kisskh.ovh"
        
        // 1. Query Sanitization (Membuang Subjudul TMDB)
        val searchKeyword = title.substringBefore(":").trim()
        val searchUrl = "$apiUrl/api/DramaList/Search?q=${searchKeyword.replace(" ", "%20")}&type=0"
        
        val searchRes = app.get(searchUrl, referer = "$apiUrl/").text
        val items = tryParseJson<Array<KisskhMedia>>(searchRes)?.toList() ?: return
        
        // 2. Alphanumeric Cleanup
        val cleanSearchTitle = title.replace(Regex("[^A-Za-z0-9]"), "").lowercase()
        val cleanOrigTitle = originalTitle?.replace(Regex("[^A-Za-z0-9]"), "")?.lowercase()
        
        // Filter awal untuk mencegah spam API detail, ambil max 3 kandidat teratas
        val potentialMatches = items.filter { item ->
            val cleanItemTitle = item.title?.replace(Regex("[^A-Za-z0-9]"), "")?.lowercase() ?: ""
            cleanItemTitle.contains(cleanSearchTitle) || cleanSearchTitle.contains(cleanItemTitle) ||
            (cleanOrigTitle != null && (cleanItemTitle.contains(cleanOrigTitle) || cleanOrigTitle.contains(cleanItemTitle)))
        }.take(3)
        
        var matchedDetail: KisskhMediaDetail? = null
        for (item in potentialMatches) {
            val detail = app.get("$apiUrl/api/DramaList/Drama/${item.id}?isq=false", referer = "$apiUrl/").parsedSafe<KisskhMediaDetail>() ?: continue
            val itemYear = detail.releaseDate?.split("-")?.firstOrNull()?.toIntOrNull()
            val cleanItemTitle = detail.title?.replace(Regex("[^A-Za-z0-9]"), "")?.lowercase() ?: ""
            
            // 4. Year-Gated Validation
            val isYearMatch = year == null || itemYear == year
            
            // 3. Two-Way Partial Match
            val isTitleMatch = cleanItemTitle.isNotEmpty() && cleanSearchTitle.isNotEmpty() && 
                    (cleanItemTitle == cleanSearchTitle || 
                    ((cleanItemTitle.contains(cleanSearchTitle) || cleanSearchTitle.contains(cleanItemTitle)) && isYearMatch))
            
            val isOriginalTitleMatch = cleanOrigTitle != null && cleanItemTitle.isNotEmpty() && cleanOrigTitle.isNotEmpty() && 
                    (cleanItemTitle == cleanOrigTitle || 
                    ((cleanItemTitle.contains(cleanOrigTitle) || cleanOrigTitle.contains(cleanItemTitle)) && isYearMatch))
            
            val isTypeMatch = if (isTvSeries) {
                detail.type != "Movie" && detail.type != "Hollywood" && detail.type != "Bollywood"
            } else {
                detail.type == "Movie" || detail.type == "Hollywood" || detail.type == "Bollywood" || (detail.episodes?.size == 1)
            }
            
            if ((isTitleMatch || isOriginalTitleMatch) && isTypeMatch) {
                matchedDetail = detail
                break
            }
        }
        
        if (matchedDetail == null) return
        
        // Cari ID spesifik episode untuk Kisskh
        val targetEpNumber = if (isTvSeries) (episode ?: 1).toDouble() else 1.0
        val targetEpsId = matchedDetail.episodes?.find { it.number == targetEpNumber }?.id ?: return
        
        // Ambil Kunci dari Macro Script
        ...
        val kkey = app.get("$KisskhAPI$targetEpsId&version=2.8.10", timeout = 10000).parsedSafe<KisskhKey>()?.key ?: ""
        val source = app.get("$apiUrl/api/DramaList/Episode/$targetEpsId.png?err=false&ts=null&time=null&kkey=$kkey", referer = "$apiUrl/").parsedSafe<KisskhSources>()
        ...
        if (source != null) {
            listOf(source.video, source.thirdParty).forEach { link ->
                ...
                if (link.contains(".m3u8")) {
                    callback.invoke(newExtractorLink(this.name, "Kisskh", link, ExtractorLinkType.M3U8) {
                        this.referer = "$apiUrl/"
                        this.headers = mapOf("Origin" to apiUrl)
                    })
                } else if (link.contains("mp4")) {
                    callback.invoke(newExtractorLink(this.name, "Kisskh", link, ExtractorLinkType.VIDEO) {
                        this.referer = apiUrl
                        this.quality = Qualities.P720.value
                    })
                } else {
                    try {
                        loadExtractor(link.substringBefore("=http"), "$apiUrl/", subtitleCallback, callback)
                    } catch (e: Exception) { logError(e) }
                }
            }
        }
        
        // Injeksi Link Subtitle
        val kkey1 = app.get("$KisskhSub$targetEpsId&version=2.8.10", timeout = 10000).parsedSafe<KisskhKey>()?.key ?: ""
        val subRes = app.get("$apiUrl/api/Sub/$targetEpsId?kkey=$kkey1").text
        tryParseJson<List<KisskhSubtitle>>(subRes)?.forEach { sub ->
            val langName = if (sub.label == "Indonesia") "Indonesian" else sub.label ?: "Unknown"
            // Penting: Subtitle .txt akan ditangkap oleh Interceptor di AdiXtream.kt
            subtitleCallback.invoke(newSubtitleFile(langName, sub.src ?: return@forEach))
        }
    }

    // Objek dekripsi AES bawaan Kisskh yang sudah digabungkan
    object KisskhDecryptor {
