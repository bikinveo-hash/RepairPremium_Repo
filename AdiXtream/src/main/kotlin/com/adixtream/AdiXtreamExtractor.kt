...
object AdiXtreamExtractor : AdiXtream() {
    ...
    suspend fun invokeKisskh(
        ...
        val kisskhUrl = "https://kisskh.ovh"
        ...
        try {
            ...
            val searchRes = app.get(
                "$kisskhUrl/api/DramaList/Search?q=${searchKeyword.replace(" ", "%20")}&type=0",
                referer = "$kisskhUrl/"
            ).parsedSafe<Array<KisskhSearchResult>>() ?: return

            // ── 2. Alphanumeric Cleanup ────────────────────────────────────────
            val cleanTmdbTitle = title.replace(Regex("[^A-Za-z0-9]"), "").lowercase()
            val cleanOrigTitle = originalTitle?.replace(Regex("[^A-Za-z0-9]"), "")?.lowercase()

            // ── 3 & 4. Two-Way Partial Match + Year-Gated Validation ──────────
            val matchedMedia = searchRes.find { item ->
                val cleanApiTitle = item.title?.replace(Regex("[^A-Za-z0-9]"), "")?.lowercase() ?: return@find false
                val apiYear = item.releaseDate?.take(4)?.toIntOrNull()

                val isExactTitle   = cleanApiTitle == cleanTmdbTitle
                val isPartialTitle = cleanApiTitle.contains(cleanTmdbTitle) || cleanTmdbTitle.contains(cleanApiTitle)

                val isExactOrig    = cleanOrigTitle != null && cleanApiTitle == cleanOrigTitle
                val isPartialOrig  = cleanOrigTitle != null &&
                        (cleanApiTitle.contains(cleanOrigTitle) || cleanOrigTitle.contains(cleanApiTitle))

                // Exact match → tidak perlu validasi tahun
                // Partial match → WAJIB validasi tahun
                val isTitleMatch = isExactTitle ||
                        (isPartialTitle && (year == null || apiYear == null || apiYear == year))
                val isOrigMatch  = isExactOrig ||
                        (isPartialOrig  && (year == null || apiYear == null || apiYear == year))

                isTitleMatch || isOrigMatch
            } ?: return

            val mediaId = matchedMedia.id ?: return

            // ── Ambil detail untuk dapat episode list ─────────────────────────
            val detail = app.get(
                "$kisskhUrl/api/DramaList/Drama/$mediaId?isq=false",
                referer = "$kisskhUrl/Drama/${matchedMedia.title?.replace(Regex("[^a-zA-Z0-9]"), "-")}?id=$mediaId"
            ).parsedSafe<KisskhMediaDetail>() ?: return

            // ── Cari episode yang sesuai ──────────────────────────────────────
            val targetEpNumber = episode?.toDouble() ?: 1.0
            val targetSeason   = season ?: 1

            // Kisskh tidak punya konsep season di episode list-nya.
            // Untuk multi-season, episode dinomori kontinu (mis. S2E1 = ep 13 jika S1 ada 12 ep).
            // Kita ambil episode dengan number yang cocok langsung jika season == 1 atau null,
            // dan fallback ke pencarian episode ID langsung untuk season > 1.
            val targetEp = if (targetSeason <= 1) {
                detail.episodes?.find { it.number?.toDouble() == targetEpNumber }
            } else {
                // Untuk season > 1, hitung offset berdasarkan total episode season sebelumnya.
                // Karena kita tidak punya data per-season dari Kisskh, gunakan angka episode as-is.
                detail.episodes?.find { it.number?.toDouble() == targetEpNumber }
            } ?: return

            val epsId = targetEp.id ?: return

            // ── Fetch key lalu stream ─────────────────────────────────────────
            val kkey = app.get("$KisskhAPI$epsId&version=2.8.10", timeout = 10000)
                .parsedSafe<KisskhKey>()?.key ?: ""

            val titleSlug = matchedMedia.title?.replace(Regex("[^a-zA-Z0-9]"), "-") ?: ""
            val epRef = "$kisskhUrl/Drama/$titleSlug/Episode-${targetEp.number?.let { if (it % 1.0 == 0.0) it.toInt().toString() else it.toString() }}?id=$mediaId&ep=$epsId&page=0&pageSize=100"

            val sources = app.get(
                "$kisskhUrl/api/DramaList/Episode/$epsId.png?err=false&ts=null&time=null&kkey=$kkey",
                referer = epRef
            ).parsedSafe<KisskhSources>() ?: return

            listOf(sources.video, sources.thirdParty).forEach { link ->
                val url = link ?: return@forEach
                when {
                    url.contains(".m3u8") -> {
                        com.lagradost.cloudstream3.utils.M3u8Helper.generateM3u8(
                            ...
                            url,
                            referer = "$kisskhUrl/",
                            headers = mapOf("Origin" to kisskhUrl)
                        ).forEach(callback)
                    }
                    url.contains("mp4")   -> {
                        callback.invoke(
                            newExtractorLink("Kisskh", "Kisskh", url, com.lagradost.cloudstream3.utils.INFER_TYPE) {
                                this.referer = kisskhUrl
            ...
            // ── Subtitle ──────────────────────────────────────────────────────
            val kkeySub = app.get("$KisskhSub$epsId&version=2.8.10", timeout = 10000)
                .parsedSafe<KisskhKey>()?.key ?: ""

            app.get("$kisskhUrl/api/Sub/$epsId?kkey=$kkeySub").text.let { subRes ->
                com.lagradost.cloudstream3.utils.AppUtils.tryParseJson<List<KisskhSubtitle>>(subRes)?.forEach { sub ->
                    val lang = when (sub.label) {
                        "Indonesia" -> "Indonesian"
                        else -> sub.label ?: return@forEach
                    }
