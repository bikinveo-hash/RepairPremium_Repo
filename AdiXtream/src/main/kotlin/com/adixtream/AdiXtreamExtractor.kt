...
object AdiXtreamExtractor : AdiXtream() {
    ...
    suspend fun invokeKisskh(
        ...
        val searchUrl = "$kisskhUrl/api/DramaList/Search?q=$title&type=0"
        
        // 1. Pencarian Awal
        val searchRes = app.get(searchUrl, referer = "$kisskhUrl/").text
        val searchItems = tryParseJson<List<KisskhSearchMedia>>(searchRes) ?: return
        
        val cleanSearchTitle = title.replace(Regex("[^A-Za-z0-9]"), "").lowercase()
        val cleanOrigTitle = originalTitle?.replace(Regex("[^A-Za-z0-9]"), "")?.lowercase()

        // 2. Filter Kandidat Judul
        val candidateMedias = searchItems.filter { item ->
            val cleanItemTitle = item.title?.replace(Regex("[^A-Za-z0-9]"), "")?.lowercase() ?: ""
            val isTitleMatch = cleanItemTitle.isNotEmpty() && cleanSearchTitle.isNotEmpty() &&
                    (cleanItemTitle == cleanSearchTitle || cleanItemTitle.contains(cleanSearchTitle))
            val isOrigMatch = cleanOrigTitle != null && cleanItemTitle.isNotEmpty() && cleanOrigTitle.isNotEmpty() &&
                    (cleanItemTitle == cleanOrigTitle || cleanItemTitle.contains(cleanOrigTitle))
            isTitleMatch || isOrigMatch
        }

        var matchedDetail: KisskhDetail? = null
        var matchedMediaId: Int? = null

        // 3. Verifikasi Tahun Rilis (Smart Search)
        for (media in candidateMedias) {
            val mediaId = media.id ?: continue
            val detailUrl = "$kisskhUrl/api/DramaList/Drama/$mediaId?isq=false"
            val detailRes = app.get(detailUrl, referer = "$kisskhUrl/Drama/Title?id=$mediaId").parsedSafe<KisskhDetail>() ?: continue
            
            val itemYear = detailRes.releaseDate?.split("-")?.firstOrNull()?.toIntOrNull()
            
            if (year == null || itemYear == year) {
                matchedDetail = detailRes
                matchedMediaId = mediaId
                break
            }
        }

        if (matchedDetail == null || matchedMediaId == null) return

        // 4. Sinkronisasi Episode
        // Catatan: Jika Movie, Kisskh biasanya menggunakan eps.number = 1.0
        val targetEpNumber = (episode ?: 1).toDouble()
        val matchedEpisode = matchedDetail.episodes?.find { it.number == targetEpNumber } ?: return
        val epsId = matchedEpisode.id ?: return

        // 5. Bypass Kunci (kkey) & Ambil Sumber Video
        val scriptApi = "https://script.google.com/macros/s/AKfycbzn8B31PuDxzaMa9_CQ0VGEDasFqfzI5bXvjaIZH4DM8DNq9q6xj1ALvZNz_JT3jF0suA/exec?id="
        val kkey = app.get("$scriptApi$epsId&version=2.8.10", timeout = 10000).parsedSafe<KisskhKey>()?.key ?: ""
        
        val sourceUrl = "$kisskhUrl/api/DramaList/Episode/$epsId.png?err=false&ts=null&time=null&kkey=$kkey"
        val sourceRes = app.get(sourceUrl, referer = "$kisskhUrl/").parsedSafe<KisskhSources>()

        listOfNotNull(sourceRes?.video, sourceRes?.thirdParty).forEach { link ->
            if (link.contains(".m3u8")) {
                M3u8Helper.generateM3u8("Kisskh", fixUrl(link), referer = "$kisskhUrl/", headers = mapOf("Origin" to kisskhUrl)).forEach(callback)
            } else if (link.contains("mp4")) {
                callback.invoke(newExtractorLink("Kisskh", "Kisskh MP4", fixUrl(link), INFER_TYPE) {
                    this.referer = kisskhUrl
                    this.quality = Qualities.P720.value
                })
            } else {
                loadExtractor(link.substringBefore("=http"), "$kisskhUrl/", subtitleCallback, callback)
            }
        }

        // 6. Ambil Subtitle
        val scriptSub = "https://script.google.com/macros/s/AKfycbyq6hTj0ZhlinYC6xbggtgo166tp6XaDKBCGtnYk8uOfYBUFwwxBui0sGXiu_zIFmA/exec?id="
        val kkeySub = app.get("$scriptSub$epsId&version=2.8.10", timeout = 10000).parsedSafe<KisskhKey>()?.key ?: ""
        val subUrl = "$kisskhUrl/api/Sub/$epsId?kkey=$kkeySub"
        tryParseJson<List<KisskhSubtitle>>(app.get(subUrl).text)?.forEach { sub ->
