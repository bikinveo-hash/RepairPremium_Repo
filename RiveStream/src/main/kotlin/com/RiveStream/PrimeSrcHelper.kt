    ...
    private suspend fun emitSource(src: ScrapperSource, url: String, callback: (ExtractorLink) -> Unit) {
        ...
        callback(
            newExtractorLink(
                ...
                // CDN video (ipcloud/ngcloud/bellevianetwork/dll) kemungkinan besar cek Referer
                // terhadap domain UI utama (tempat user "menonton"), bukan domain API backend
                // scrapper.rivestream.app. Kalau headers eksplisit (flowcast) tersedia, pakai itu;
                // kalau tidak, fallback ke RIVE_BASE alih-alih SCRAPPER_BASE.
                this.referer = headers["Referer"] ?: RIVE_BASE
                // Attach headers lain kalau ada (Origin, dll)
                this.headers = headers.filterKeys { it != "Referer" } + mapOf("User-Agent" to USER_AGENT)
